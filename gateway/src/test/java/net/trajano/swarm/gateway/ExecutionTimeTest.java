...
class ExecutionTimeTest {
  ...
  void reportExecutionTimeTest() {
    var stream = events().flatMap(rec -> processEvent(rec).transform(this::reportExecutionTime));

    StepVerifier.create(stream).expectNextCount(100).verifyComplete();
  }
  ...
  private Mono<Integer> processEvent(int i) {
    return Mono.delay(Duration.ofMillis(i + 100)).thenReturn(i);
  }
}
