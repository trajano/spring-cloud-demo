...
public class AuthController<A, R, P> {

    private final AuthService<A, R, P> authService;

    @RequestMapping(path = "${auth.controller-mappings.username-password-authentication:/auth}",
            method = RequestMethod.POST)
    public Mono<R> authenticate(A authenticationRequest, @RequestHeader Map<String, String> headers, ServerHttpResponse serverHttpResponse) {
    ...
    @RequestMapping(path = "${auth.controller-mappings.refresh:/refresh}",
            method = RequestMethod.POST,
            consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE
    )
    public Mono<R> refreshUrlEncoded(@ModelAttribute OAuthRefreshRequest oAuthRefreshRequest, @RequestHeader Map<String, String> headers, ServerHttpResponse serverHttpResponse) {

        if (!oAuthRefreshRequest.getGrant_type().equals("refresh_token")) {
            throw new IllegalArgumentException();
        }
        final var refreshResponse = authService.refresh(oAuthRefreshRequest.getRefresh_token(), headers);
    ...
    @RequestMapping(path = "${auth.controller-mappings.logout:/logout}",
            method = RequestMethod.POST,
            consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE
    )
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public Mono<Void> logout(@ModelAttribute OAuthRevocationRequest oAuthRefreshRequest, @RequestHeader Map<String, String> headers, ServerHttpResponse serverHttpResponse) {
