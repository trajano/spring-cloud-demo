...
/**
 * Interface representing the auth data storage. The interface allows for an
 * altnerate storage implementation to be used.
 */
export interface IAuthStore {
  /**
   * Obtains the access token provide if it is available and not expired. This
   * will return null if the token is expired. This is exposed so if we have to
   * do "background fetch".
   *
  ...
  /**
   * Obtains the full OAuth token provide if it is available. This does not
   * check expiration.
   *
