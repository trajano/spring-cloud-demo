...
it("should i18n Text and preseve other styles and remap font", async () => {
    ...
    const { toJSON: toExpectedJson } = render(<RNText testID="eval" style={{ backgroundColor: "red", fontFamily: 'IBMPlexSans_400Regular', color: "#FFFFFF" }}>hello</RNText>);
...
it("should i18n Text and preseve other styles and remap font while wrapped in a view", async () => {
    ...
    const { getByTestId, toJSON, unmount } = render((
        <ThemeProvider
            ...
            translations={{ "en": { key: "hello" } }}
        >
            <View backgroundColor="red"><Text testID="eval" fontFamily="IBMPlexSans" _t="key" _tp={{ prop: "val", prop2: "val" }} /></View>
        </ThemeProvider>))
    const { toJSON: toExpectedJson } = render(<RNView style={{ backgroundColor: "red" }}><RNText testID="eval" style={{ color: "#FFFFFF", fontFamily: 'IBMPlexSans_400Regular' }}>hello</RNText></RNView>);
