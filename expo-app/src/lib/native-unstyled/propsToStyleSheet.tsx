...
function doPropsToStyleSheet(
...
): StyleProp<Record<string, unknown>> | undefined {
  ...
  function add(propKey: string, styleKey: string) {
    if (
      ...
      (props as Record<string, unknown>)[propKey] !== null
    ) {
  ...
  function addColor(propKey: string, styleKey: string) {
    if (
      ...
      const evaluatedColor = lookupColor(
        (props as Record<string, unknown>)[propKey] as RN.ColorValue,
        colorSchemeColors
      );
...
export const propsToStyleSheet = memoize(
  doPropsToStyleSheet,
  (props, colorSchemeColors) =>
 ...
 * @returns
 */
export function withoutStyledProps<P extends object>(props: P) {
  if (__DEV__) {
