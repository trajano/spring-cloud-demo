...
describe("markdownToTextElements", () => {
  it("empty string", () => {
    const { toJSON: expectedToJSON } = render(<Text />);
  ...
  it("bold-italic", () => {
    const { toJSON } = render(
      inlineMarkdownToTextElements("***bold-italic***")!
    );
    const { toJSON: expectedToJSON } = render(
      <Text style={{ fontStyle: "italic" }}>
        <Text style={{ fontWeight: "bold" }}>bold-italic</Text>
      </Text>
    );
    expect(toJSON()).toStrictEqual(expectedToJSON());
  });
  it("complex", () => {
    const { toJSON } = render(
      inlineMarkdownToTextElements("foo **bold _italic_** foo adf *asdf*")!
    );
    const { toJSON: expectedToJSON } = render(
      <Text>
        {"foo "}
